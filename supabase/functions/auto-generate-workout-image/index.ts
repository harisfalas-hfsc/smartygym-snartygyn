...
serve(async (req) => {
  ...
  try {
    ...
    const isMicro = workout.category?.toUpperCase() === "MICRO-WORKOUTS";
    ...
    if (isMicro) {
      visualDirection = `
- Show a real-life "exercise snack" scene that captures "${workout.name}" — a 5-minute workout someone does WITHOUT going to a gym
