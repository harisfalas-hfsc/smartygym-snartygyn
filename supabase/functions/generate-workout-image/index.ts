...
- Show elegant, controlled Pilates poses: planks, leg lifts, spine stretches, reformer exercises`;
    } else if (categoryUpper === "RECOVERY") {
      // NOTE: Recovery workouts have NO difficulty level - suitable for everyone
      visualDirection = `
- Show a gentle recovery/regeneration scene that captures the essence of "${name}"
...
Remember: ZERO text or writing of any kind on the image. Only show people exercising or fitness equipment/environments.`;
    ...
    try {
      ...
      console.log(`Optimized workout image: ${originalBuffer.length} -> ${jpegBytes.length} bytes`);
    } catch (optErr) {
      console.warn("Image optimization failed, falling back to original PNG:", optErr);
    }

    const fileName = `workout-${Date.now()}-${Math.random().toString(36).substring(7)}.${uploadExt}`;
