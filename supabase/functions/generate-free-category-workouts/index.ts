...
${category === "CHALLENGE" ? `CHALLENGE: a benchmark feel. For Time or AMRAP. High demand but completable by intermediate athletes. HARD BAN in Main Workout and Finisher: stretching, mobility, yoga poses, static flexibility drills, or recovery exercises. Challenge means high-output reps, rounds, time pressure, and discomfort tolerance — not stretching.` : ""}
...
const AI_MODELS = ["google/gemini-2.5-pro", "openai/gpt-5-mini"];
...
async function generateOne(
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      content.main_workout = repairStaticHoldPrescriptions(content.main_workout, `[FREE-HOLD-RX][${equipment}]`).processedContent;
      content.main_workout = removeStaticHoldsFromMomentumSections(content.main_workout, `[FREE-HOLD-PLACEMENT][${equipment}]`).processedContent;
