...
BEGIN
  IF current_setting('request.jwt.claim.role', true) IS DISTINCT FROM 'service_role'
    ...
    RAISE EXCEPTION 'Only admins can view cron heartbeat status';
  END IF;
...
BEGIN
  IF current_setting('request.jwt.claim.role', true) IS DISTINCT FROM 'service_role'
     AND NOT public.has_role(auth.uid(), 'admin') THEN
    RAISE EXCEPTION 'Only admins can sync cron metadata';
  END IF;
  ...
  WITH latest_runs AS (
    ...
    WHERE m.job_name = l.jobname
      ...
      AND (
        ...
        OR m.last_run_at IS DISTINCT FROM l.start_time
