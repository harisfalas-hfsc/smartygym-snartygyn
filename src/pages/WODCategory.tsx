...
const WODCategory = () => {
  ...
  const wodInteraction = currentWOD ? interactions.find(i => i.workout_id === currentWOD.id) : null;
      ...
      {isLoading ? <div className="min-h-screen bg-background py-8 px-4">
          ...
          <div className="min-h-screen bg-background">
            ...
            label: "WOD"
              ...
              <Card className="mb-8 bg-gradient-to-br from-primary/5 via-background to-primary/5 border-2 border-primary/40 shadow-gold">
                <div className="p-4 sm:p-6">
                  ...
                  <div className="space-y-2 text-muted-foreground max-w-3xl mx-auto">
                    <p className="text-sm sm:text-base text-center">Every day SmartyGym delivers a fresh, expertly designed workout following a strategic periodization cycle. Each day focuses on a different training style: Strength, Calorie Burning, Metabolic, Cardio, Mobility & Stability, and Challenge, rotating continuously for balanced, progressive training. It's like going to the gym and following the workout of the day but you can do it at your local gym, at home, in your office, outdoors, or even while traveling.<span className="text-primary font-semibold"> SmartyGym: the gym that never closes and never takes a holiday</span> delivers a fresh, expertly designed workout following a strategic periodization cycle. Each day focuses on a different training style: <strong>Strength</strong>, <strong>Calorie Burning</strong>, <strong>Metabolic</strong>, <strong>Cardio</strong>, <strong>Mobility & Stability</strong>, and <strong>Challenge</strong> — rotating continuously for balanced, progressive training. It's like going to the gym and following the workout of the day — but you can do it at your local gym, at home, in your office, outdoors, or even while traveling.
                    </p>
                  </div>
                </div>
              </Card>
              ...
              {currentWOD ? <Card className="group cursor-pointer overflow-hidden hover:shadow-xl transition-all duration-300 hover:border-primary/60" onClick={() => navigate(`/workout/wod/${currentWOD.id}`)}>
                  {/* Large Image */}
                  <div className="relative aspect-video overflow-hidden">
                    <img src={currentWOD.image_url || "/placeholder.svg"} alt={currentWOD.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />

                    {/* NEW Badge */}
                    {isNew(currentWOD.created_at) && <Badge className="absolute top-3 left-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0">
                        NEW
                      </Badge>}

                    {/* Status badges for logged-in users */}
                    {userId && wodInteraction && <div className="absolute top-3 right-3 flex gap-2">
                        {wodInteraction.is_completed && <Badge className="bg-green-500/90 text-white border-0">
                            <Check className="w-3 h-3 mr-1" />
                            Completed
                          </Badge>}
                        {wodInteraction.has_viewed && !wodInteraction.is_completed && <Badge className="bg-blue-500/90 text-white border-0">
                            <Eye className="w-3 h-3 mr-1" />
                            Viewed
                          </Badge>}
                        {wodInteraction.is_favorite && <Badge className="bg-pink-500/90 text-white border-0">
                    ...
                    <div className="absolute bottom-3 right-3">
                      {currentWOD.is_premium ? <Badge className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white border-0 shadow-lg">
                          <Crown className="w-3 h-3 mr-1" />
                          Premium
                        </Badge> : currentWOD.is_standalone_purchase && currentWOD.price ? <Badge className="bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0 shadow-lg">
                  ...
                  <CardContent className="p-6 sm:p-8">
                    ...
                    {/* Description */}
                    {currentWOD.description && <p className="text-muted-foreground mb-6 text-center max-w-2xl mx-auto line-clamp-3">
                        {stripHtml(currentWOD.description).substring(0, 200)}...
                      </p>}

                    {/* Metadata Badges */}
                    <div className="flex flex-wrap justify-center gap-2 mb-6">
                      {currentWOD.category && <Badge variant="outline" className="text-sm">
                          <Flame className="w-3 h-3 mr-1" />
                          {currentWOD.category}
                        </Badge>}
                      {currentWOD.format && <Badge variant="outline" className="text-sm">
                          {currentWOD.format}
                        </Badge>}
                      {currentWOD.difficulty && <Badge variant="outline" className="text-sm">
                          <Star className="w-3 h-3 mr-1" />
                          {currentWOD.difficulty}
                        </Badge>}
                      {currentWOD.equipment && <Badge variant="outline" className="text-sm">
                          <Dumbbell className="w-3 h-3 mr-1" />
                          {currentWOD.equipment}
                        </Badge>}
                      {currentWOD.duration && <Badge variant="outline" className="text-sm">
