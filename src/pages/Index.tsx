...
const Index = () => {
  ...
  const [isPwaInstalled, setIsPwaInstalled] = useState(true); // Default true to hide until checked
  ...
  // PWA detection and install prompt listener
  useEffect(() => {
  ...
  const handleInstallPWA = async () => {
    if (isAndroid && deferredPrompt) {
      ...
      try {
        ...
        if (outcome === "accepted") {
          toast({ title: "Success!", description: "SmartyGym has been installed" });
      ...
      // iOS: Show instructions (no native API)
      toast({
      ...
      toast({
        ...
        description: "Please access SmartyGym from your mobile device to install the app.",
        duration: 5000,
      });
    }
  };
  ...
  return <>
      ...
      <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        {/* Mobile Only: Download App Icons - Only shows on mobile when not installed as PWA */}
        {isMobile && !isPwaInstalled && (
          <div className="mt-6 mb-2 px-4 text-center">
            {/* Two icons side by side */}
            <div className="flex justify-center items-center gap-6">
              {/* Android Play Store Icon - Clickable */}
              <button
                onClick={() => isAndroid ? handleInstallPWA() : toast({
                  title: "Android Device Required",
                  description: "This download is for Android devices only.",
                  duration: 3000,
                })}
                className="w-12 h-12 rounded-xl bg-green-500/10 hover:bg-green-500/20 flex items-center justify-center transition-colors"
              ...
              {/* iOS App Store Icon - Clickable */}
              <button
                onClick={() => isIOS ? handleInstallPWA() : toast({
                  title: "iOS Device Required", 
                  description: "This download is for iPhone/iPad only.",
                  duration: 3000,
                })}
            ...
            {/* Simple text below icons */}
            <p className="mt-3 text-sm font-medium text-muted-foreground">
              Download SmartyGym app
            </p>
          </div>
        )}
          </section> : <>
