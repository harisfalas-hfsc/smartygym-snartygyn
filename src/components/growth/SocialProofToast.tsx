...
const WORKOUT_NAMES = [
  "Upper Body Blast", "Core Ignition", "Leg Day Power", "Cardio Rush",
  "Total Body Forge", "Mobility Flow", "HIIT Crusher", "Strength Builder",
  "Metabolic Engine", "Calorie Torch"
];

const MESSAGES = [
  () => `${pick(FIRST_NAMES)} just completed "${pick(WORKOUT_NAMES)}"`,
  () => `${pick(FIRST_NAMES)} from ${pick(CITIES)} just signed up`,
  () => `${randInt(8, 25)} people started a training program today`,
  () => `${pick(FIRST_NAMES)} just earned their first workout badge`,
  () => `${randInt(5, 15)} workouts completed in the last hour`,
  () => `${pick(FIRST_NAMES)} rated "${pick(WORKOUT_NAMES)}" ⭐⭐⭐⭐⭐`,
  () => `New member ${pick(FIRST_NAMES)} from ${pick(CITIES)} joined SmartyGym`,
  () => `${pick(FIRST_NAMES)} just unlocked a premium workout`,
];
