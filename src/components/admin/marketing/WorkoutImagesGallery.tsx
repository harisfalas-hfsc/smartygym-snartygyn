...
export const WorkoutImagesGallery = () => {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('admin-workout-images-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        () => {
          queryClient.invalidateQueries({ queryKey: ["admin-workout-images"] });
        }
      )
      .subscribe();
  ...
  const handleDownload = async (workout: Workout) => {
    ...
    const size = selectedSizes[workout.id] || INSTAGRAM_SIZES[0];
    try {
      await downloadImageAsInstagramSize(
        workout.image_url,
        workout.name.replace(/\s+/g, "-").toLowerCase(),
        size
      );
      toast.success(`Downloaded ${workout.name}`);
    } catch (error) {
  ...
  const handleDownloadAll = async () => {
    ...
    const workoutsWithImages = workouts.filter(w => w.image_url);
    toast.loading(`Downloading ${workoutsWithImages.length} images...`, { id: "download-all" });
    
    let successCount = 0;
    for (const workout of workoutsWithImages) {
      try {
        await handleDownload(workout);
        successCount++;
    ...
    toast.success(`Downloaded ${successCount}/${workoutsWithImages.length} images!`, { id: "download-all" });
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">
          {workoutsWithImages.length} workout images available
        </p>
        ...
        {workoutsWithImages.map((workout) => (
          <Card key={workout.id} className="overflow-hidden">
            ...
            <CardContent className="p-2 space-y-2">
              <div>
                <h4 className="text-xs font-medium truncate" title={workout.name}>
                  {workout.name}
                </h4>
                {workout.category && (
              ...
              <div className="flex gap-1">
                <Select
                  value={selectedSizes[workout.id]?.name || "square"}
                  onValueChange={(value) => {
                    const size = INSTAGRAM_SIZES.find(s => s.name === value);
                    if (size) {
                      setSelectedSizes(prev => ({ ...prev, [workout.id]: size }));
                    }
                  }}
                >
