import { Clock, TrendingUp, Target, Layers } from "lucide-react";
...
export const WorkoutInfoBar = ({ duration, difficulty, focus, category }: WorkoutInfoBarProps) => {
  const getDifficultyColor = (diff: string) => {
    const lower = diff.toLowerCase();
    if (lower.includes("beginner")) return "bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20";
    if (lower.includes("intermediate")) return "bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20";
  ...
  return (
    <div className="flex flex-wrap gap-2 p-3 bg-card border border-border rounded-lg mb-6">
      {category && (
        <Badge variant="outline" className={`flex items-center gap-1 px-2 py-1 ${getCategoryColor(category)}`}>
      ...
      <Badge variant="outline" className={`flex items-center gap-1 px-2 py-1 ${getDifficultyColor(difficulty)}`}>
        ...
        <span className="text-[10px] font-medium">{difficulty}</span>
      </Badge>
      <Badge variant="outline" className="flex items-center gap-1 px-2 py-1">
