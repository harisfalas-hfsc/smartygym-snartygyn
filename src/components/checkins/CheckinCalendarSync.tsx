...
export function CheckinCalendarSync() {
  ...
  const checkStatus = async () => {
    try {
      ...
      const { data: connection } = await supabase
        ...
        .select('is_active, checkin_reminder_event_ids')
        .eq('user_id', user.id)
        .eq('provider', 'google')
        .maybeSingle();
      ...
      if (connection?.is_active) {
        ...
        setIsSynced(!!eventIds?.morning_event_id && !!eventIds?.night_event_id);
      }
      setLoading(false);
    } catch (error) {
    ...
    try {
      ...
      if (isSynced) {
        ...
        const { data, error } = await supabase.functions.invoke('sync-google-calendar', {
          body: { action: 'delete-checkin-reminders' }
        });
        ...
        if (data?.success) {
          setIsSynced(false);
          toast.success('Check-in reminders removed from Google Calendar');
        } else {
        ...
        const { data, error } = await supabase.functions.invoke('sync-google-calendar', {
          body: { action: 'create-checkin-reminders' }
        });
        ...
        if (data?.success) {
          setIsSynced(true);
          toast.success('Check-in reminders added to Google Calendar (8:00 AM & 8:00 PM daily)');
        } else {
          ...
          <div className="flex items-center gap-3 text-center sm:text-left">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {isSynced 
                  ? 'Daily reminders at 8:00 AM & 8:00 PM in your calendar' 
                  : 'Add recurring check-in reminders to your calendar'}
              </p>
            </div>
          </div>
          <Button
