...
export const WorkoutOfTheDay = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("admin_workouts").select("*").eq("is_workout_of_day", true).single();
  ...
  const renderStars = (count: number | null) => {
    ...
    return <span className="flex items-center gap-0.5">
        {Array.from({
        length: count
      }).map((_, i) => <Star key={i} className="w-3 h-3 fill-current" />)}
      </span>;
  };
  if (isLoading) {
    return <Card className="mb-8 bg-gradient-to-br from-primary/10 via-background to-primary/10 border-2 border-primary/50 shadow-gold overflow-hidden">
  ...
  return <Card className="mb-8 bg-gradient-to-br from-primary/10 via-background to-primary/10 border-2 border-primary/50 shadow-gold overflow-hidden">
      <div className="p-4 sm:p-6">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          {/* Description */}
          <p className="text-sm sm:text-base text-muted-foreground max-w-3xl mx-auto mb-6">Every day at 7:00 AM, Your SmartyGym: the gym that never closes and never takes a holiday. delivers a fresh, expertly designed workout following a strategic periodization cycle. Each day focuses on a different training style: Strength, Calorie Burning, Metabolic, Cardio, Mobility & Stability, and Challenge rotating continuously for balanced, progressive training. It's like going to the gym and following the workout of the day — but you can do it at your local gym, at home, in your office, outdoors, or even while traveling. Your SmartyGym: the gym that never closes and never takes a holiday.<span className="text-primary font-semibold">
Your SmartyGym: the gym that never closes and never takes a holiday.</span> delivers a fresh, expertly designed workout following a strategic periodization cycle. Each day focuses on a different training style: <strong>Strength</strong>, <strong>Calorie Burning</strong>, <strong>Metabolic</strong>, <strong>Cardio</strong>, <strong>Mobility & Stability</strong>, and <strong>Challenge</strong> — rotating continuously for balanced, progressive training. It's like going to the gym and following the workout of the day — but you can do it at your local gym, at home, in your office, outdoors, or even while traveling. <strong>Your <span className="text-primary">SmartyGym</span>: the gym that never closes and never takes a holiday.</strong>
        ...
        {/* Current WOD Display */}
        {wod ? <div className="bg-background/80 backdrop-blur-sm rounded-xl p-4 sm:p-5 border border-primary/30 max-w-2xl mx-auto">
            {/* WOD Image */}
            {wod.image_url && <div className="relative w-full h-40 sm:h-48 rounded-lg overflow-hidden mb-4">
                <img src={wod.image_url} alt={wod.name || "Workout of the Day"} className="w-full h-full object-cover" />
            ...
            {/* WOD Details */}
            {!wod.image_url && <h3 className="text-lg sm:text-xl font-bold text-center mb-3">
                {wod.name}
              </h3>}
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
              ...
              {/* Format Badge */}
              {wod.format && <Badge variant="outline" className="bg-blue-500/20 text-blue-700 dark:text-blue-400 border-blue-500/40">
                  {wod.format}
                </Badge>}
              ...
              {/* Duration Badge */}
              {wod.duration && <Badge variant="outline" className="bg-purple-500/20 text-purple-700 dark:text-purple-400 border-purple-500/40">
            ...
            <div className="flex justify-center">
              <Button onClick={() => navigate(`/workout/${wod.category?.toLowerCase().replace(/ & /g, "-").replace(/ /g, "-")}/${wod.id}`)} className="cta-button">
                View Today's Workout
              </Button>
            </div>
          </div> : <div className="bg-background/80 backdrop-blur-sm rounded-xl p-6 border border-primary/30 max-w-md mx-auto text-center">
        ...
        <div className="text-center mt-4">
          <Button variant="link" onClick={() => navigate("/wod-archive")} className="text-muted-foreground hover:text-primary">
