    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className={`flex items-center justify-between px-4 py-3 border-b ${
          isDarkMode ? 'border-zinc-700 bg-zinc-800' : 'border-amber-200 bg-amber-100'
        }`}>
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              className={`h-8 w-8 ${isDarkMode ? 'hover:bg-zinc-700 text-zinc-300' : 'hover:bg-amber-200 text-zinc-700'}`}
              title="Decrease font size"
            >
              <Minus className="h-4 w-4" />
            </Button>
            <span className={`text-xs min-w-[40px] text-center ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>
              {fontSize}px
            </span>
            <Button
              variant="ghost"
            ...
            <div className={`w-px h-6 mx-2 ${isDarkMode ? 'bg-zinc-600' : 'bg-amber-300'}`} />
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 md:px-12">
          <article className="max-w-3xl mx-auto">
            {/* Title */}
            <h1 
              className={`font-bold mb-6 leading-tight ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}
            ...
            {/* Metadata Bar */}
            {metadata && (
              <div className={`flex flex-wrap gap-3 mb-8 pb-6 border-b text-sm ${
                isDarkMode ? 'border-zinc-700 text-zinc-400' : 'border-amber-200 text-zinc-600'
              }`}>
                {metadata.author && (
                  <span>By <strong className={isDarkMode ? 'text-zinc-300' : 'text-zinc-800'}>{metadata.author}</strong></span>
                )}
